...
def vis_graf():
    ...
    landliste = []
    målingsdata = {måling: [] for måling in valgte_målinger}
    ...
    for land in flestytland.keys():
        ...
        for måling in valgte_målinger:
            gjennomsnitt = landinfo[måling].mean()
            målingsdata[måling].append(round(gjennomsnitt))

    # Plot
    x = np.arange(len(landliste))
    bredde = 0.8 / len(valgte_målinger)

    fig, ax = plt.subplots(figsize=(12, 6))
    for i, måling in enumerate(valgte_målinger):
        offset = (i - len(valgte_målinger) / 2) * bredde + bredde / 2
        ax.bar(x + offset, målingsdata[måling], width=bredde, label=måling)

    ax.set_xticks(x)
    ax.set_xticklabels(landliste, rotation=45)
    ax.set_title("Gjennomsnitt per kanal")
    ax.set_ylabel("Verdi")
    ax.set_xlabel("Land")
    ax.legend()
