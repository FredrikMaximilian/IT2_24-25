...
file_name_json = "Datasett_fodselstall.json"
this_path = os.path.abspath(os.path.dirname(__file__))
path_csv = os.path.join(this_path, file_path,file_name_csv)
path_json = os.path.join(this_path, file_path,file_name_json)

df = pd.read_csv(path_csv)
print(df.loc[0])
print(df.keys()[0])

#keys = ['År','Levendefødte','Innflyttinger','Utflyttinger']
#df.keys(keys)
#df.index(['År','Levendefødte','Innflyttinger','Utflyttinger'])

#"År","Levendef�dte i alt","Innflyttinger","Utflyttinger"
#Oppgave a
df['Netto folkevekst'] = df['Levendefødte i alt']+df['Innflyttinger']-df['Utflyttinger']
kategorier = ['Levendefødte i alt', 'Innflyttinger','Utflyttinger']

pd.options.display.max_rows = 9999
print(df)

"""TKinter kommer til å lage et eget vindu,
og pygame kan kun ha ett vindu så her blir det kræsj

Se heller tkpyplot.py
...
matplotlib.use('Agg')
#Les mer om dette her: https://matplotlib.org/stable/users/explain/figure/backends.html
...
# Data representert som liste med ordbøker
# "årstall" og "representanter" er nøklene, og "representanter" er en ordbok med partienes mandater
